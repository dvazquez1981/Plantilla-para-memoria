...
En la figura~\ref{fig:phpmyadmin_medicion} se muestra la tabla \texttt{Medicion} en phpMyAdmin, con los registros almacenados en la base de datos MySQL. Cada uno es una detección vehicular procesada por el sistema, con su fecha, valor, carril y clasificación. Esto evidencia que el backend recibió y registró correctamente las mediciones enviadas por el nodo, incluso tras interrupciones de red.
\vspace{-0.5em}
\begin{figure}[H]
  \centering
...
En este caso, el backend ejecuta un comando remoto dirigido al dispositivo, publicado en el tópico MQTT \texttt{dispositivo/{id}/comando} y el nodo recibe dicho comando, lo procesa localmente y responde en el tópico
\texttt{dispositivo/{id}/respuesta}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.60\linewidth]{./Figures/phpmyadmin_comando_respuesta.png}
...
Las pruebas se realizaron sobre el endpoint \texttt{POST /api/comando}, verificando tanto la creación exitosa como la validación de datos.
...
En la figura~\ref{fig:postman_comando_error} se observa la respuesta del sistema ante una solicitud con parámetros faltantes.  
\vspace{-0.5em}
\begin{figure}[H]
  \centering
