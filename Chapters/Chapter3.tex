...
\begin{itemize}
  ...
  \texttt{POST /app/comando} al backend, que crea un \texttt{comm\_id} único con dispostivoId y publica en \texttt{dispositivo/{id}/comando}.
...
Cada vez que un dispositivo publica una medicion en \texttt{dispositivo/\{id\}/medicion}, el backend lo recibe, valida el payload y lo persiste en MySQL.
 
En el backend, los comandos se publican en \texttt{dispositivo/\{id\}/comando}  y las respuestas se actualizan según los envíos en \texttt{dispositivo/\{id\}/respuesta} incluyendo en el body el cmd\_id. 
...
\begin{itemize}
  ...
  \item RespuestaController: centraliza la recepción de estados y telemetría (batería, conectividad), garantizando que la base de datos refleje la situación en tiempo real.
...
A continuación, se presenta el mapa general de los endpoints y controladores mas importantes para el flujo:
